...
eval_dataset_ceilnet = datasets.CEILTestDataset(join(datadir, 'real_test'), size=20)
...
#eval_dataset_real = datasets.CEILTestDataset(join(datadir, 'real20'), fns=read_fns('real_test.txt'), size=20)
...
if len(opt.gpu_ids) > 0:
  test_eval_dataset_ceilnet = datasets.DataLoader(
       eval_dataset_ceilnet, batch_size=1, shuffle=False,
       num_workers=opt.nThreads, pin_memory=True)
else:
  test_eval_dataset_ceilnet = datasets.DataLoader(
...
res = engine.test(test_eval_dataset_ceilnet, savedir=join(result_dir, 'test_eval_dataset_ceilnet'))

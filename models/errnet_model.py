...
class ERRNetBase(BaseModel):
    ...
    def set_input(self, data, mode='train'):
        ...
        #    print("slided input[", i, "]:", select_input.shape)'''
    ...
    def print_network(self):
        if self.opt.debug:
            ...
            if self.vgg is not None:
                ...
                summary(self.vgg, input_size=(3, 224, 224))#torch vgg model summary for 224*224 i/p size
            print('##################### NetG #####################')
    ...
    def load(model, resume_epoch=None):
        ...
        if icnn_path is None:
            ...
            if len(model.opt.gpu_ids) > 0:
                ...
                state_dict = torch.load(icnn_path, map_location=lambda storage, loc: storage)
            model.net_i.load_state_dict(state_dict['icnn'])
            model.epoch = state_dict['epoch']
            model.iterations = state_dict['iterations']
            # if model.isTrain:
